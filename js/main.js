...
formRsvp.addEventListener('submit', async e => {
  ...
  try {
    const res = await fetch('https://xv-isa-backend-production-36cf.up.railway.app/rsvp', {
      method: 'POST',
...
formCanciones.addEventListener('submit', async e => {
  ...
  try {
    const res = await fetch('https://xv-isa-backend-production-36cf.up.railway.app/canciones', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
